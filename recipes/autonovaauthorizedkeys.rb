...
if Chef::Config[:solo]
  ...
  Chef::Log.info("osops-utils/autonovaauthorizedkeys: Compiling list of host DSA keys")
  ...
  nodes.each do |node|
    ...
    unless known_hosts.include?(host_line)
      Chef::Log.info("osops-utils/autonovaauthorizedkeys: Adding host DSA key for #{node["fqdn"]}")
      known_hosts << host_line
    end
  end

  Chef::Log.info("osops-utils/autonovaauthorizedkeys: Writing /var/lib/nova/.ssh/known_hosts")
